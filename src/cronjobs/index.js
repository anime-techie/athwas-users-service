...
module.exports = {
      ...
      if (result && result.length > 0) {
        await result.reduce(async (promise, orderdata, key) => {
          ...
          await UserServices.updateRecord(
            find,
            {
              $inc: { earned_point: +parseInt(normal_setting.reward_points) },
              last_earned_point_date: new Date(),
            },
            "id wallet username"
          )
              ...
              if (updatedWallet) {
                var createRecord = {
                  ...
                  expiration_date: new Date(
                    new Date().getTime() +
                      normal_setting.expiration_time * 24 * 60 * 60 * 1000
                  ),
                };
                await RedeemPointServices.createRecord(createRecord)
  ...
  notifyMe: async () => {
    await ProductInventoryServices.allRecord([
      {
        $match: {
          notify_me: { $exists: true, $gt: { $size: 0 } },
      ...
      if (result && result.length > 0) {
        await result.reduce(async (promise, productdata, key) => {
          ...
          await productdata.notify_me.reduce(async (promise, userdata, key) => {
            ...
            let userInfo = await UserServices.getUserDetailById({
              _id: userdata.user_id,
            });
            if (userInfo.device_token != "") {
              var productname = await ProductInventoryServices.oneRecord([
                { $match: { _id: ObjectID(productdata._id) } },
                {
                  $lookup: {
                    from: "products",
              ...
              await NotificationServices.createRecord(createNotification).then(
                async (result) => {
                  return true;
                }
              );
          ...
          await ProductInventoryServices.updateRecord(newfindPattern, {
            notify_me: [],
          }).then((updatedRes) => {
            return true;
          });
  ...
  updateDealofDay: async () => {
    ...
    var aggregate = [
      {
        $match: {
          statusactiveDate: { $lt: new Date(Date.now() - 1000 * 60 * 60 * 24) },
          is_active: 1,
          is_deleted: 0,
        },
      },
    ...
    if (alldealofdayIds.length > 0) {
      await DealOfDayServices.updateManyRecord(filterbyids, { is_active: 0 })
        .then((resut) => {
  ...
  minimumInventory: async () => {
    ...
    await ProductInventoryServices.allRecord([
      {
        $match: {
          $expr: { $eq: ["$product_quantity", "$min_inventory"] },
          last_notification_time: { $ne: toDay },
        },
      },
    ])
      .then(async (result) => {
        ...
        if (result && result.length > 0) {
          await result.reduce(async (promise, productdata, key) => {
            ...
            var message =
              "Product " +
              productname.inventory_name +
              " reach at their  min inventory limit";
          ...
          await ProductInventoryServices.updateManyRecord(filterbyids, {
            last_notification_time: toDay,
          })
            .then((resut) => {
    ...
    await RedeemPointServices.allRecord([
      ...
      .then(async (result) => {
        ...
        if (redeemPointsIds.length > 0) {
          ...
          await RedeemPointServices.updateManyRecord(
            filterbyids,
            [
              {
                $set: { redeem_points: "$earned_points", expiration_date: "" },
              },
            ],
            { multi: true }
          );
