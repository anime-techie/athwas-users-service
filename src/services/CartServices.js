...
const CartServices = {
  createOrUpdateRecord: async function (Records, quantity) {
    var aggregate = [{ $match: Records }];
    return await Carts.aggregate(aggregate)
      .then(async (cartData) => {
        ...
        if (cartData && cartData.length > 0) {
          ...
          return await Carts.create(Records)
            .then((cartData) => {
  ...
  allRecord: async function (condition = {}) {
    return await Carts.aggregate(condition)
      .then((cartData) => {
        return cartData;
      })
      .catch((err) => {
  ...
  deleteRecord: async function (condition) {
    return Carts.deleteOne(condition)
      .then((deleteRes) => {
        return deleteRes;
      })
      .catch((err) => {
  ...
  deleteAllRecord: async function (condition) {
    return Carts.deleteMany(condition)
      .then((deleteRes) => {
        return deleteRes;
      })
      .catch((err) => {
  ...
  getDataCount: async function (findPattern) {
    return Carts.countDocuments(findPattern)
      .then((count) => {
        return count;
      })
      .catch((err) => {
  ...
  getAggregatePaginatedData: async function (
    ...
    return await Carts.aggregatePaginate(myAggregate, options)
