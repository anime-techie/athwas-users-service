...
const UserServices = {
  ...
  allRecord: async (condition = {}) => {
    return await Users.aggregate(condition)
      .then((createRes) => {
        return createRes;
      })
      .catch((err) => {
  ...
  updateRecord: async function (findPattern, updatePattern, customselect = "") {
    console.log(findPattern, "========>", updatePattern);
    var options = {
      new: true,
      select:
        customselect != ""
          ? customselect
          : "user_image_url user_image email _id phone email country_code username auth_token is_user_verified is_email_verified is_phone_verified allow_notifications delivery_address online_status user_role",
    };
    return Users.findOneAndUpdate(findPattern, updatePattern, options)
      .then((updatedData) => {
        console.log("dfjgdfjgf================***********");
        return updatedData;
      })
      .catch((err) => {
        throw err;
  ...
  getPaginatedData: async function (findPattern, sortPattern, page_no, limit) {
    ...
    return await Users.aggregatePaginate(myAggregate, options)
  ...
  getDataCount: async function (findPattern) {
    return Users.countDocuments(findPattern)
      .then((count) => {
        return count;
      })
      .catch((err) => {
  ...
  getUserDetailById: async function (findPattern) {
    return Users.findOne(findPattern)
      .then((userDetail) => {
        return userDetail;
      })
      .catch((err) => {
  ...
  updateManyRecord: async (condition, data) => {
    return await Users.update(condition, data)
      .then((createRes) => {
        return createRes;
      })
      .catch((err) => {
