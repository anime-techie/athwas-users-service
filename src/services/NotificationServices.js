...
const NotificationServices = {
  createRecord: async function (Records) {
    return await Notifications.create(Records)
  ...
  allRecord: async function (condition = {}) {
    return await Notifications.findAll(condition)
      .then((notificationData) => {
        return notificationData;
      })
      .catch((err) => {
  ...
  oneRecord: async function (condition = []) {
    return await Notifications.aggregate(condition)
      .then((notificationData) => {
        return notificationData;
      })
      .catch((err) => {
  ...
  deleteAllRecord: async function (condition) {
    return Notifications.deleteMany(condition)
      .then((deleteRes) => {
        return deleteRes;
      })
      .catch((err) => {
  ...
  getAggregatePaginatedData: async function (
    ...
    return await Notifications.aggregatePaginate(myAggregate, options)
  ...
  getDataCount: async function (findPattern) {
    return Notifications.countDocuments(findPattern)
      .then((count) => {
        return count;
      })
      .catch((err) => {
  ...
  updateRecord: async (data, condition) => {
    return await Notifications.findOneAndUpdate(data, condition, { new: true })
      .select("")
      .then((createRes) => {
        return createRes;
      })
  ...
  updateRecordMany: async (data, condition) => {
    return await Notifications.updateMany(data, condition, { new: true })
      .select("")
      .then((createRes) => {
        return createRes;
      })
    ...
    if (user_id && user_id != "") {
      ...
      if (notification_type.includes(2)) {
        if (
          ...
          console.log("messgae send ", JSON.stringify(Fcmmessage));
      ...
      //-----------------------------------Normal-------------------------------//
      if (
        notification_type.includes(3) &&
        ((userInfo && userInfo.is_deleted == 0) || user_type == 1)
      ) {
        console.log("createnotification======");
        await NotificationServices.createRecord(createNotification)
          .then(async (result) => {
            let resMsg = "Success";
          })
          .catch((err) => {
