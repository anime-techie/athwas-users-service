...
const ProductServices = {
  ...
  oneRecord: async (condition = {}) => {
    return await Product.aggregate(condition)
      .then((createRes) => {
        return createRes;
      })
      .catch((err) => {
  ...
  updateRecord: async (data, condition) => {
    return await Product.findOneAndUpdate(data, condition, { new: true })
      .select("")
      .then((createRes) => {
        return createRes;
      })
  ...
  updateUserImage: async function (itemData, imageData, deleteExiting = true) {
    if (imageData) {
      ...
      return await ProductImageServices.createRecord(createRecord).then(
        (result) => {
          return result._id;
        }
      );
  ...
  getAggregatePaginatedData: async function (
    ...
    return await Product.aggregatePaginate(myAggregate, options)
      .then(async (paginatedData) => {
        return paginatedData;
      })
      .catch((err) => {
  ...
  genrateproductcode: async function (productId) {
    ...
    return await ProductServices.oneRecord(aggregateCondition)
      .then(async (result) => {
        result = result[0];
        var random = Common.generateRandomNumber(4);
        return (
          result.businessCategoryData.name.substring(0, 3) +
          "-" +
          result.CategoryData.name.substring(0, 3) +
          "-" +
          result.SubCategoryData.name.substring(0, 3) +
          "-" +
          random +
          "-" +
  ...
  getDataCount: async function (findPattern) {
    return Product.countDocuments(findPattern)
      .then((count) => {
        return count;
      })
      .catch((err) => {
