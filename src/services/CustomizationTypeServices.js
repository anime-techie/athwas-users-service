...
const CustomizationTypeServices = {
  createRecord: async (categoryData) => {
    return await CustomizationType.create(categoryData)
      .then((createRes) => {
        return createRes;
      })
      .catch((err) => {
  ...
  allRecord: async (condition = {}) => {
    return await CustomizationType.aggregate(condition)
      .then((createRes) => {
        return createRes;
      })
      .catch((err) => {
  ...
  updateRecord: async function (findPattern, updatePattern) {
    ...
    return CustomizationType.findOneAndUpdate(
      findPattern,
      updatePattern,
      options
    )
  ...
  getPaginatedData: async function (findPattern, sortPattern, page_no, limit) {
    ...
    return await CustomizationType.aggregatePaginate(aggregate, options)
  ...
  deleteRecord: async function (deletePattern) {
    return CustomizationType.deleteOne(deletePattern)
      .then((createRes) => {
        return createRes;
      })
      .catch((err) => {
