...
const OrderServices = {
  processOrder: async function (updaterecord, user_id) {
    ...
    let checkInventory = await OrderServices.checkProductInventory(user_id);
    console.log("------test222222----------",checkInventory)
    if(checkInventory.status){
      ...
      console.log("------test----------",checkInventory)
      throw checkInventory.message;
    }

  },

  checkProductInventory: async function (user_id){
  ...
  orderDetail: async function (query) {
    ...
    return await Order.aggregate([
      ...
      { $unwind: "$products" },
      {
        ...
        {
          from: 'products',
          let: { "id": "$products.product_id","inventory_id":"$products.inventory_id","oid":"$_id" },
          pipeline: [
            { $match: { $expr: { $eq: ["$_id", "$$id"] } } },
            {
              $lookup: {
                  from: 'product_inventories',
                  let: { "id": "$$inventory_id","oid":"$$oid" },
                  pipeline: [
                      { $match: { $expr: { $eq: ["$_id", "$$id"] } } },
                      {
                        $project:{
                          ...
                          review:{
                            $filter:{
                              input:"$ratings",
                              as:"num",
                              cond:{$eq:["$$num.order_id","$$oid"]}
                            }
                          }
                        }
                      },
                      ],
                      as: 'productInventryData'
                  }
            },
            {$unwind:'$productInventryData'},
            {
                  $lookup: {
                      from: 'categories',
                      let: { "id": "$category_id" },
