...
const UPLOAD_FOLDER_URL =
  ...
  "/service/assets?service=user_service&filepath=uploads/offers";
const DEFAULT_FOLDER_URL =
  config.apiGatewayUrl +
  "/service/assets?service=user_service&filepath=default";
const UPLOAD_FOLDER_PRODUCT_URL =
  config.apiGatewayUrl +
...
const OfferController = {
  ...
  applyOffer: async (req, res) => {
    try {
      ...
      let offerData = await OfferServices.oneRecord([
        {
          $match: {
      ...
      let resultProduct = cartData.map((a) => String(a.product_id._id));
      let offerProduct =
        offerData &&
        offerData.productInventoriesData.length > 0 &&
        offerData.productInventoriesData.map((a) => String(a.product_id));

      let compareResult = function () {
        let result = false;
        for (var i = 0, len = offerProduct.length; i < len; i++) {
          if (resultProduct.includes(offerProduct[i])) {
            result = true;
          } else {
            result = false;
            break;
      ...
      if (
        (offerData.offer_type == 2 || offerData.offer_type == 3) &&
        // resultProduct.length != offerProduct.length &&
        !compareResult()
      ) {
        var errMessage = Messages.PRODUCT_CART_IS_NOT_VALID;
        Response.send(req, res, 400, errMessage);
      } else {
        if (cartData && cartData.length > 0) {
          ...
          cartData = await cartData.map((item) => {
            ...
            if (item.is_discount === 1) {
              ...
              order_amount += item.inventory_id.price * item.quantity;
            }
          ...
          var login_user_id =
            Common.getUserdetailsBytoken(req.headers.authorization) || null;
          OfferServices.applyOffer(req.body, login_user_id)
            .then(async (orderResult) => {
              if (orderResult) {
                if (orderResult.offer_type == 1) {
                  if (orderResult.offer_amount <= order_amount) {
                    let newamount = orderResult.offer_price;
                    if (orderResult.offer_amount_type == 2) {
                      newamount =
                        (order_amount * orderResult.offer_price) / 100;
                    }
                    var result = {
                      id: orderResult._id,
                      discount_price: newamount,
                      promo_code: req.body.promo_code,
                      offer_type: orderResult.offer_type,
                    };
                    var succMessage = Messages.OFFER_APPLIED_SUCCSSFULLY;
                    Response.send(req, res, 200, succMessage, result);
                  } else {
                    var errMessage =
                      "Promocode applicable for Cart Value of Rs." +
                      orderResult.offer_amount +
                      " and above";
                    Response.send(req, res, 400, errMessage);
                  }
                } else if (orderResult.offer_type == 2) {
                  var amount = await OfferController.checkBundle(
                    cartData,
                    orderResult
                  );
                  console.log(amount, "==========", orderResult.offer_amount);
                  if (amount > 0) {
                  ...
                  var offerValid = await OfferController.checkPromitioanl(
                    cartData,
                    orderResult
                  );
                  ...
                  if (offerValid) {
                    ...
                    var succMessage =
                      "Add " +
                      orderResult.offer_quantity +
                      " quantity of product to apply promocode on this product";
                    // var succMessage = Messages.PROMOCODE_NOT_APPLIED;
                    Response.send(req, res, 400, succMessage, orderResult);
                  }
                }
  ...
  checkPromitioanl: async (cartdata, offerdata) => {
    ...
    if (offerdata.business_category != "") {
      var businesscategory = cartdata.filter(
        (o) =>
          o.business_category_id.toString() ===
          offerdata.business_category_id.toString()
      );
      ...
      await businesscategory.map((value) => {
        ...
        if (value && offerdata.category_id != "") {
          if (
            value.product_id.category_id.toString() ===
            offerdata.category_id.toString()
          ) {
            if (offerdata.sub_category_id != "") {
              if (
                value.product_id.sub_category_id.toString() ===
                offerdata.sub_category_id.toString()
              ) {
                if (offerdata.product_id != "") {
                  if (
                    value.inventory_id._id.toString() ===
                    offerdata.product_id.toString()
                  ) {
                    result = true;
                  }
                } else {
            ...
            if (value.inventory_id._id != "") {
              ...
              if (index >= 0) {
                ...
                if (value.inventory_id.is_discount == 1) {
                  totalproductprice +=
                    value.inventory_id.discounted_product_price;
                } else {
  ...
  getOffers: async (req, res) => {
    try {
      ...
      //   ];
      if (keyword && keyword != "") {
        findPattern["$or"] = [{ title: { $regex: keyword, $options: "i" } }];
      }
      // if (status && status != "") {
      ...
      if (product_id && product_id != "") {
        productfilter = {
          $expr: { $in: [ObjectID(product_id), "$product_id"] },
        };
      }
      ...
      OfferServices.getAggregatePaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
