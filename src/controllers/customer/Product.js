...
const {
  ...
  CategoryServices,
  FavouriteItemServices,
  ProductServices,
  BrandServices,
  ProductInventoryServices,
  BannerServices,
  ProductCustomizationServices,
  OrderServices,
  DealOfDayServices,
} = require("../../services/");
const {
...
const UPLOAD_BANNER_FOLDER_URL =
  config.apiGatewayUrl +
  "/service/assets?service=user_service&filepath=uploads/banners";
const UPLOAD_DEALOFDAY_FOLDER_URL =
  config.apiGatewayUrl +
  "/service/assets?service=user_service&filepath=uploads/dealofday";
...
*/
  ...
  getBusinesscategory: (req, res) => {
    try {
      ...
      BusinessCategoryServices.getPaginatedData(
...
*/
  ...
  getCategory: (req, res) => {
    try {
      ...
      CategoryServices.getPaginatedData(
        findCategory,
        sortPattern,
        1,
        100,
...
*/
  ...
  getFavouriteItems: (req, res) => {
    try {
      var user_id =
        Common.getUserdetailsBytoken(req.headers.authorization) || "";
      ...
      var aggregateCondition = [
...

        {
          $project: {
            ...
            is_active: "$inventoryproducts.Products.is_active",
          },
        },
      ];
      ...
      FavouriteItemServices.getAggregatePaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
  ...
  addFavouriteItem: async (req, res) => {
    try {
      ...
      if (errors.length > 0) {
        ...
        FavouriteItemServices.oneRecord(createPattern)
          .then((favResult) => {
            if (favResult) {
              var updateRecord = {
                is_active: status,
              };
              FavouriteItemServices.updateRecord(
                { _id: favResult._id },
                updateRecord
              )
              ...
              FavouriteItemServices.createRecord(createPattern)
                .then(async (createRes) => {
                  var findPattern = { _id: createRes._id };
                  var updatePattern = {};

                  FavouriteItemServices.updateRecord(findPattern, updatePattern)
                    .then((updatedRes) => {
                      // success
  ...
  getProducts: async (req, res) => {
    try {
      ...
      if (brand_id && brand_id != "") {
        otherfilter["$and"] = [{ "Products.brand_id": ObjectId(brand_id) }];
      }
      var pricefilter = {};
      if (price_start != "" && price_end != "") {
        ...
        pricefilter["$and"] = [{ newprice: { $gte: parseInt(price_start) } }];
      } else if (price_end != "") {
        pricefilter["$and"] = [{ newprice: { $lte: parseInt(price_end) } }];
      }
      ...
      let allProductIds = await ProductCustomizationServices.allRecord(
        aggregateConditionForproduct
      );
      ...
      let aggregateCondition = [
        ...
        { $sort: sortPattern },
        {
          $project: {
            ...
            // name: { $ltrim: { input: "$Products.name" } },
            name: "$Products.name",
      ...
      ProductInventoryServices.getAggregatePaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
...
*/
  getProductDetail: async (req, res) => {
    try {
      ...
      var aggregatefilter = [
                      ...
                      $project: {
                        product_image_url: {
                          $cond: [
                            ...
                            {
                              $concat: [
                                DEFAULT_FOLDER_URL + "/placeholder-user.jpg",
                              ],
                            },
                            {
                              $concat: [
                ...
                $lookup: {
                  ...
                  pipeline: [
                    ...
                    { $unwind: "$ProductCustomizationValueData" },
                    {
                      $project: {
                        name: 1,
                        value: "$ProductCustomizationValueData",
                        parent_id: 1,
                      },
                    },
                  ],
        ...
        {
          $project: {
            name: { $ltrim: { input: "$ProductsData.name" } },
              ...
              $cond: {
                if: {
                  $and: [
                    {
                      $and: [
                        { $eq: ["$ProductsData.is_active", 1] },
                        { $eq: ["$ProductsData.is_deleted", 0] },
                      ],
                    },
                    {
                      $and: [
                        {
                          $eq: [
                            "$ProductsData.businessCategoryData.is_active",
                            1,
                          ],
                        },
                        {
                          $eq: [
                            "$ProductsData.businessCategoryData.is_deleted",
                            0,
                          ],
                        },
                      ],
                    ...
                    {
                      $and: [
                        { $eq: ["$ProductsData.CategoryData.is_active", 1] },
                        { $eq: ["$ProductsData.CategoryData.is_deleted", 0] },
                      ],
                    },
                    {
                      $and: [
                        { $eq: ["$ProductsData.SubCategoryData.is_active", 1] },
                        {
                          $eq: ["$ProductsData.SubCategoryData.is_deleted", 0],
                        },
                      ],
                    },
                  ],
      ...
      ProductInventoryServices.oneRecord(aggregatefilter)
        .then((productData) => {
          if (productData) {
            let resMsg = "Success";
            var newproductData = {
  ...
  getBrands: (req, res) => {
    try {
      ...
      BrandServices.getPaginatedData(findPattern, sortPattern, page, limit)
        .then((branddata) => {
          let resMsg = "Success";
...
*/
  getBannerList: async (req, res) => {
    try {
      ...
      let aggregateCondition = [
        ...
        // },
        {
          $project: {
            ...
            banner_image_url: {
              $concat: [
                UPLOAD_BANNER_FOLDER_URL + "/",
                { $toString: "$_id" },
                "/",
                "$banner_image",
              ],
            },
            banner_image_thumb_url: {
              $concat: [
                UPLOAD_BANNER_FOLDER_URL + "/",
                { $toString: "$_id" },
                "/thumb_",
      ...
      BannerServices.getAggregatePaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
    ...
    try {
      ...
      let aggregateCondition = [
...

        {
          $project: {
            _id: "$_id",
            name: { $ltrim: { input: "$productData.name" } },
  ...
  OtherProductList: async (req, res) => {
    try {
        ...
        { $unwind: "$productData" },
        {
          $lookup: {
            ...
            pipeline: [
              {
                $match: {
                  $expr: { $eq: ["$product_inventry_id", "$$id"] },
                  is_deleted: 0,
                },
              },
              {
      ...
      ProductInventoryServices.allRecord(
        aggregateCondition,
        sortPattern,
        page,
        limit
...
*/
  notifyMe: async (req, res) => {
    ...
    var findPattern = { _id: ObjectId(product_id) };
    var aggregate = [{ $match: findPattern }];

    ProductInventoryServices.oneRecord(aggregate)
      .then((cartResult) => {
        if (cartResult) {
          var createPatteren = {
          ...
          var newfindPattern = {
            ...
            "notify_me.user_id": { $ne: ObjectId(login_user_id) },
          };

          ProductInventoryServices.updateRecord(newfindPattern, {
            $push: { notify_me: createPatteren },
          })
            .then((updatedRes) => {
  ...
  getCustomizationType: async (req, res) => {
    try {
      ...
      if (category_id && category_id == "") {
        errors.push({
          errField: "category_id",
          errText: Messages.INVALID_IMAGE_FORMAT,
        });
      }
      ...
      if (errors.length > 0) {
        ...
        ProductCustomizationServices.getAggregatePaginatedData(
          aggregateCondition,
          sortPattern,
          page,
          limit
  ...
  getCustomizationSubType: async (req, res) => {
    try {
      ...
      if (errors.length > 0) {
        ...
        var page = parseInt(req.query.page_no) || 1;
        var limit = parseInt(req.query.limit) || 10;
        ProductCustomizationServices.allRecord(aggregateCondition)
          .then((resultProduct) => {
            var result = {
...
*/
  getDealOfDayList: async (req, res) => {
    try {
      ...
      DealOfDayServices.getAggregatePaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
  ...
  getDealOfDayDetails: async (req, res) => {
    try {
      ...
      if (dealofday_id && dealofday_id == "") {
        errors.push({
          errField: "dealofday_id",
          errText: Messages.INVALID_DEALOFDAY_REQUEST,
        });
      }
      ...
      if (errors.length > 0) {
        ...
        DealOfDayServices.oneRecord(aggregateCondition)
          .then((userdata) => {
            let resMsg = "Success";
            Response.send(req, res, 200, resMsg, userdata);
          })
