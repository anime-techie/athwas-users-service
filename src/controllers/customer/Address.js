        ...
        UserServices.oneRecord(aggregatefilter)
          .then(async (userdata) => {
            if (userdata) {
              ...
              UserServices.updateRecord(findPattern, {
                $push: { delivery_address: delivery_address },
              })
                .then((updatedRes) => {
                  // success
                  let resMsg = Messages.NEW_ADDRESS;
                  Response.send(
                    req,
                    res,
                    200,
                    resMsg,
                    updatedRes.delete_address
                  );
                })
                .catch((err) => {
                  let resMsg = Validation.getErrorMessage(err);
        ...
        .then(async (addressdata) => {
          if (addressdata != null) {
            UserServices.updateRecord(
              {
                _id: addressdata._id,
                "delivery_address._id": ObjectID(address_id),
              },
              { $set: { "delivery_address.$.is_deleted": 1 } }
            )
              .then((userObjRes) => {
  ...
  set_default_address: (req, res) => {
    try {
      ...
      UserServices.oneRecord(aggregatefilter)
        .then(async (addressdata) => {
          if (addressdata != null) {
            await UserServices.updateManyRecord(
              {
                _id: ObjectID(addressdata._id),
                delivery_address: {
                  $elemMatch: { _id: { $ne: ObjectID(address_id) } },
                },
              },
              { $set: { "delivery_address.$[].default_address": 0 } },
              { multi: true }
            );
            ...
            UserServices.updateRecord(
              ...
              { $set: { "delivery_address.$.default_address": 1 } }
            )
              .then((userObjRes) => {
