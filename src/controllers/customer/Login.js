...
*/
  ...
  login: (req, res) => {
    ...
    try {
      ...
      UserServices.oneRecord(aggregatecondition)
        .then(async (userResult) => {
          if (userResult) {
            if (userResult.email == phone) {
              if (
                userResult.is_active == 0 &&
                userResult.is_email_verified == 1
              ) {
                ...
                await UserServices.updateRecord(findPattern, updateEmail)
                  .then((userResult) => {
                    var customData = {};
                    ...
                    customData.link =
                      ...
                      encryptDataID.iv +
                      "-" +
                      encryptDataID.encryptedData +
                      "/" +
                      encryptDataToken.iv +
                      "-" +
                      encryptDataToken.encryptedData;
                    customData.siteTitle = config.siteTitle;
                    var filePath =
                      config.rootPath +
                      "/src/views/email_templates/send_verification_email.ejs";
                    console.log(userResult.email, customData);
                    ejs.renderFile(filePath, customData, (err, data) => {
                      if (err) {
                        throw err;
                      } else {
                        var subject = "Verify Email";
                        SendMail.sendEmail(userResult.email, subject, data);
                      }
                    });
                  })
                  .catch((err) => {
            ...
            var CheckPassword = Encryption.compareBcryptEncryption(
              password,
              userResult.password
            );
            if (CheckPassword) {
              if (
                ...
                OtpServices.saveAndSendOtp(createOtpPattern).then(
                  (otpResult) => {
                    ...
                    SendSMS.sendTwilioSMS(
                      "+" + country_code + phone + "",
                      resMsg + ": " + otpResult.otp_number + " " + message_id
                    );
                ...
                UserServices.updateRecord(
                  ...
                  var key =
                    updatedUserData.user_role + "_" + updatedUserData._id;
                  Redis.hmset(
                    ["_auth_tokens", key, updatedUserData.auth_token],
                    function (err, res) {
  ...
  logout: (req, res) => {
    try {
      ...
      UserServices.updateRecord(findPattern, updatePattern)
        .then((updatedUserData) => {
          if (updatedUserData) {
            console.log(updatedUserData, "updatedUserData");
            // Remove login user auth token from Redis
          ...
          Response.send(req, res, 200, Messages.LOGOUT_SUCCESS);
        })
        .catch((err) => {
...
*/
  ...
  customer_register: async (req, res) => {
    try {
      ...
      await UserServices.oneRecord(aggregatecondition)
        .then(async (userdata) => {
          ...
          var checkphone = await UserServices.oneRecord(
            checkforphoneaggregatecondition
          );
          ...
          var checkemail = UserServices.oneRecord(
            checkforemailaggregatecondition
          );
          if (checkemail && checkemail.is_email_verified == 1) {
            ...
            UserServices.updateRecord(findPattern, updatePattern)
              .then((userObjRes) => {
                ...
                OtpServices.saveAndSendOtp(createOtpPattern)
                  .then((otpRes) => {
                    let resMsg = Messages.VERIFY_OTP;
                    var OtpresMsg = "<#> Your OTP is";
                    SendSMS.sendTwilioSMS(
                      "+" + country_code + phone + "",
                      OtpresMsg + ": " + otpRes.otp_number + " " + message_id
                    );
            ...
            UserServices.createRecord(createPattern)
              .then((userObjRes) => {
                var createOtpPattern = {};
                      ...
                      .then(async (otpDeleteRes) => {
                        ...
                        UserServices.updateRecord(
                          ...
                          .then((updateResult) => {
                            ...
                            // Set login user auth token in Redis
                            var key =
                              updateResult.user_role + "_" + updateResult._id;
                            Redis.hmset(
                              ["_auth_tokens", key, updateResult.auth_token],
                              function (err, res) {
                                if (err) console.log(err.toString());
                              }
                            );
  ...
  resend_otp: (req, res) => {
    try {
      ...
      UserServices.oneRecord(aggregatecondition)
        .then(async (userResult) => {
          if (userResult) {
            if (userResult.is_active == 0) { //4 16
              let resMsg =
                "Your account has been deactivated by admin, contact the Admin";
              Response.send(req, res, 400, resMsg);
            }
            if (
              userResult.email == phone &&
              ...
              await UserServices.updateRecord(findPattern, userResult)
                .then((userResult) => {
                  var customData = {};
                  customData.name = userResult.username;
                  customData.link =
              ...
              OtpServices.saveAndSendOtp(createOtpPattern)
                .then(async (resultOtp) => {
                  ...
                  if (otp_for == "email_verify") {
                    ...
                    SendSMS.sendTwilioSMS(
                      "+" + country_code + phone + "",
                      OtpresMsg + ": " + resultOtp.otp_number + " " + message_id
                    );
            ...
            let resMsg =
              "Invalid Email or Phone number .No account exist with this.";
            Response.send(req, res, 400, resMsg);
          }
        })
        .catch((err) => {});
