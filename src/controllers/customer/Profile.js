            ...
            UserServices.updateRecord(findPattern, updatePattern)
              .then((updateResult) => {
                if (updateResult) {
                  ...
                  var filePath =
                    config.rootPath +
                    "/src/views/email_templates/email_verify_success.ejs";
    ...
    try {
      ...
      if (allowfields.indexOf(field_key) != -1) {
        if (field_key == "new_email" || field_key == "new_phone") {
          ...
          UserServices.oneRecord(aggregatesearchFilter).then((updateRecod) => {
            if (updateRecod) {
              if (field_key == "new_email") {
          ...
          .then(async (userResult) => {
            if (userResult) {
              ...
              if (field_key == "new_email") {
                ...
                await OtpServices.saveAndSendOtp(createOtpPattern)
                  .then((resultOtp) => {
                    var token = Common.generateRandomNumber(12);
                    ...
                    var filePath =
                      config.rootPath +
                      "/src/views/email_templates/send_otp.ejs";
              ...
              if (field_key == "user_image") {
                ...
                if (
                  files &&
                  files.field_value &&
                  Media.getMediaTypeByMimeType(files.field_value.mimetype) !=
                    "image"
                ) {
                  errors.push({
                    errField: "user_image",
              ...
              var select =
                "user_image_url user_image email _id phone email country_code username  is_user_verified is_email_verified ";
        ...
        UserServices.oneRecord(aggregateFilter)
          .then((userData) => {
            if (userData != null) {
              ...
              if (userData.is_active == 0 && userData.is_user_verified == 1) {
                let resMsg =
