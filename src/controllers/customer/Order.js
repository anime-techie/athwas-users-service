...
const OrderController = {
  ...
  userOrders: async (req, res) => {
    try {
      ...
      await OrderServices.userOrders(findPattern, sortPattern, page, limit)
        .then((orderRecords) => {
  ...
  calculateDeliveryFees: async (req, res) => {
    try {
      ...
      if (delivery_address_id == "") {
        ...
        errors.push({
          errField: "delivery_address_id",
          errText: Messages.DELIVERY_ADDRESS_NOT_FOUND,
        });
      }

      if (order_amount == "") {
        errors.push({
          errField: "order_amount",
          errText: "Order Amount is required",
        });
      }
      // return errors
      if (errors.length > 0) {
        ...
        let cartData = await CartServices.findCartItems(
          { user_id: user_id },
          {
            product_id: 1,
        ...
        let condition = [
          {
            $match: {
              ...
              delivery_address: {
                $elemMatch: { _id: ObjectID(delivery_address_id) },
              },
            },
          },
          {
        ...
        let redeem_point =
          userObject && userObject.earned_point ? userObject.earned_point : 0;
        // console.log("redeem_point",redeem_point);
        let deliveryLocation = userObject.delivery_address;

        let warehouse = await WarehouseServices.findNearest(
          deliveryLocation.geoLocation.coordinates
        );

        if (!warehouse) {
          var errMessage =
            "We don't deliver in the selected area. Please choose another address.";
          Response.send(req, res, 400, errMessage);
          return;
        } else {
          await getNearestWarehouse(cartData, warehouse, [])
            .then(async (updateRecord) => {
              //console.log("updateRecord",updateRecord);
              if (!updateRecord.exist) {
                var errMessage =
                  "Product not available please check your cart again";
                Response.send(req, res, 400, errMessage);
              } else {
                ...
                let distance = Common.distance(
                  warehouse.loc.coordinates[1],
                  warehouse.loc.coordinates[0],
                  deliveryLocation.geoLocation.coordinates[1],
                  deliveryLocation.geoLocation.coordinates[0],
                  "K"
                );
                console.log(distance);
                let delivery_fee = await ContentServices.postRequest(
                  "customer/delivery_settings",
                  { distance: parseFloat(distance) }
                );
                let normal_setting = await ContentServices.getRequest(
                  "customer/normal_settings"
                );
                if (delivery_fee) {
          ...
          if (cartData) {
            ...
            await cartData.reduce(async (promise, val, i) => {
              ...
              let isWarehouseExist = await ProductInventry.findOne({
                ...
                warehouseinventry: {
                  $elemMatch: {
                    warehouse_id: ObjectID(warehouseData._id),
                    quantity: { $gt: 0 },
                  },
                },
              });
            ...
            if (result) {
              ...
              let newWarehouse = await WarehouseServices.findNearestExcept(
                deliveryLocation.geoLocation.coordinates,
                except
              );
              if (newWarehouse == null) {
                ...
                return await getNearestWarehouse(
                  cartData,
                  newWarehouse,
                  except
                );
  ...
  processOrder: async (req, res) => {
    try {
      ...
      //console.log(1);
      let remdomDelay = await getRandomInt(1, 10);
      //console.log("remdomDelay",remdomDelay);
      await sleep(remdomDelay);
      //console.log(2);
      ...
      if (payment_mode && payment_mode == "") {
        errors.push({
          errField: "payment_mode",
          errText: Messages.PAYMENT_MODE_NOT_FOUND,
        });
      }
      if (warehouse_id && warehouse_id == "") {
        errors.push({
          errField: "warehouse_id",
          errText: Messages.WAREHOUSE_NOT_FOUND,
        });
      }
      if (delivery_address && delivery_address == "") {
        errors.push({
          errField: "delivery_address",
          errText: Messages.DELIVERY_ADDRESS_NOT_FOUND,
        });
      }
      if (delivery_fee && delivery_fee == "") {
        errors.push({
          errField: "delivery_fee",
          errText: Messages.DELIVERY_CHARGE_NOT_FOUND,
        });
      }
      if (vat_amount && vat_amount == "") {
        errors.push({
          errField: "vat_amount",
          errText: Messages.VAT_NOT_FOUND,
        });
      }
      if (errors.length > 0) {
        let resMsg = errors.pop().errText;
                ...
                $cond: {
                  if: {
                    $and: [
                      {
                        $and: [
                          { $eq: ["$ProductData.is_active", 1] },
                          { $eq: ["$ProductData.is_deleted", 0] },
                        ],
                      },
                      {
                        $and: [
                          { $eq: ["$businessCategoryData.is_active", 1] },
                          { $eq: ["$businessCategoryData.is_deleted", 0] },
                        ],
                      },
                      {
                        $and: [
                          { $eq: ["$CategoryData.is_active", 1] },
                          { $eq: ["$CategoryData.is_deleted", 0] },
                        ],
                      },
                      {
                        $and: [
                          { $eq: ["$SubCategoryData.is_active", 1] },
                          { $eq: ["$SubCategoryData.is_deleted", 0] },
                        ],
                      },
                    ],
                  },
        ...
        if (cartData && cartData.length > 0) {
          ...
          cartData = await cartData.map((item) => {
            ...
            if (item.inventory_id.is_discount == 1) {
              total_amount +=
                item.inventory_id.discounted_product_price * item.quantity;
              net_amount +=
                item.inventory_id.discounted_product_price * item.quantity;
          ...
          discount = parseFloat(discount) + parseFloat(product_discount);

          let normal_setting = await ContentServices.getRequest(
            "customer/normal_settings"
          );

          var expected_start_date = new Date();
          var expected_end_date = new Date();
          if (net_amount > normal_setting.normal_order_amount) {
            var toDay = moment();
            toDay = DateTime.convertUtcToLocalByTimezone(
              toDay,
              "Asia/Kolkata",
              "H:M"
            );
            var newDate = DateTime.convertUtcToLocalByTimezone(
              moment(),
              "Asia/Kolkata",
              "yyyy-m-d"
            );
            if (
              normal_setting.premium_start_delivery_time <= toDay &&
              normal_setting.premium_end_delivery_time >= toDay
            ) {
              expected_start_date = new Date(
                newDate +
                  " " +
                  normal_setting.premium_start_delivery_time +
                  ":00"
              );
              expected_end_date = new Date(
                newDate + " " + normal_setting.premium_end_delivery_time + ":00"
              );
            } else {
              expected_start_date = new Date(
                newDate +
              ...
              expected_start_date = new Date(
                expected_start_date.setDate(expected_start_date.getDate() + 1)
              );
              expected_end_date = new Date(
                expected_end_date.setDate(expected_end_date.getDate() + 1)
              );
            }
          } else {
            var toDay = moment();
            ...
            if (
              normal_setting.normal_start_delivery_time <= toDay &&
              normal_setting.normal_end_delivery_time >= toDay
            ) {
              expected_start_date = new Date(
                newDate +
                  " " +
                  normal_setting.normal_start_delivery_time +
                  ":00"
              );
              expected_end_date = new Date(
                newDate + " " + normal_setting.normal_end_delivery_time + ":00"
              );
            } else {
              expected_start_date = new Date(
                newDate +
          ...
          let deliveryObject = userInfo.delivery_address.find(
            (item) => item && item._id.toString() == req.body.delivery_address
          );
          ...
          let warehouse = await WarehouseServices.findNearest(
            deliveryObject.geoLocation.coordinates
          );
          await getNearestWarehouse(cartData, warehouse, [])
            .then(async (updateRecord) => {
              if (!updateRecord.exist) {
                ...
                if (!warehouse) {
                  ...
                  if (orderplace) {
                    ...
                    var updaterecord = {
                      ...
                      products: cartData,
                      delivery_fee: delivery_fee,
                      expected_start_date: DateTime.convertLocalToUtcByTimezone(
                        expected_start_date,
                        "UTC"
                      ),
                      expected_end_date: DateTime.convertLocalToUtcByTimezone(
                        expected_end_date,
                        "UTC"
                      ),
                    ...
                    if (user_order_id && user_order_id != "") {
                      //  console.log("user")
                      orderResult = await OrderServices.updateprocessOrder(
                        updaterecord,
                        { order_id: user_order_id },
                        user_id
                      );
                      ...
                      // console.log("order")
                      orderResult = await OrderServices.processOrder(
                        updaterecord,
                        user_id
                      );
                    }
                    ...
                    if (orderResult) {
                      ...
                      if (counterof_cartitem > updateRecord.itemcount) {
                        ...
                        var notificationText =
                          ...
                          " does not have complete product quantity";
                      ...
                      cartData.forEach(async (val) => {
                        let productExist =
                          await ProductInventoryServices.oneRecord([
                            {
                              $match: {
                                _id: ObjectID(val.inventory_id._id),
                                warehouseinventry: {
                                  $elemMatch: {
                                    warehouse_id: ObjectID(warehouse._id),
                                  },
                                },
                              },
                            },
                        ...
                        if (productExist) {
                          await ProductInventoryServices.updateRecord(
                            ...
                            {
                              $inc: {
                                "warehouseinventry.$.quantity": -val.quantity,
                              },
                            }
                          );
                        } else {
                          ...
                          await ProductInventoryServices.updateRecord(
                            ...
                            {
                              $push: {
                                warehouseinventry: createProductInventry,
                              },
                            }
                          );
                        }
                      ...
                      if (redeem_points && redeem_points > 0) {
                        UserServices.updateRecord(
                          { _id: ObjectID(user_id) },
                          { $inc: { earned_point: -parseFloat(redeem_points) } }
                        );
                      }
                      if (req.body.payment_mode == "Wallet") {
                        await UserServices.updateRecord(
                          { _id: ObjectID(user_id) },
                          { $inc: { wallet: -parseFloat(net_amount) } },
                          "id wallet username"
                          ...
                          if (updatedWallet) {
                            ...
                            await TransactionHistoryServices.createRecord(
                              addPattern
                            );
                          }
                        });
                    ...
                    await NotificationServices.sendNotification(
                      "Athwas",
                      "Order #" + orderResult.order_id + " Placed Successfully",
                      config.adminid,
                      1,
                      user_id,
                      3,
      ...
      let aggregateCondition = [
        {
          $match: {
            ...
            user_id: ObjectID(user_id),
            products: { $elemMatch: { _id: ObjectID(sub_order_id) } },
          },
        },
        { $unwind: "$products" },
      ];
      OrderServices.oneRecord(aggregateCondition)
        .then(async (orderDetail) => {
          ...
          if (orderDetail && orderDetail.length > 0) {
            ...
            let bussinessCatData = await BusinessCategoryServices.oneRecord({
              _id: orderdata.products.business_category_id,
            });

            let createTime = orderdata.createdAt;
            let OrderCancelTime = new Date(
              createTime.getTime() + bussinessCatData.cancelation_time * 60000
            );
                    ...
                    .then(async (orderUpdate) => {
                        ...
                        await ProductInventoryServices.oneRecord([
                          {
                            $match: {
                              _id: ObjectID(
                                updatedOrderdetails.product[0].inventory_id
                              ),
                              warehouseinventry: {
                                $elemMatch: {
                      ...
                      if (productExist) {
                        await ProductInventoryServices.updateRecord(
                          {
                            ...
                            warehouseinventry: {
                              $elemMatch: {
                                warehouse_id: ObjectID(orderdata.warehouse_id),
                              },
                            },
                          },
                          {
                            $inc: {
                              "warehouseinventry.$.quantity":
                                orderdata.products.quantity,
                            },
                          }
                        );
                      } else {
                        ...
                        await ProductInventoryServices.updateRecord(
                          ...
                          {
                            $push: { warehouseinventry: createProductInventry },
                          }
                        );
                      }
                      ...
                      if (orderUpdate.payment_mode != "Cash") {
                        var returnamount =
                          updatedOrderdetails.product[0].is_discount == 1
                            ? updatedOrderdetails.product[0].offer_price
                            : updatedOrderdetails.product[0].price;
                        returnamount =
                          returnamount *
                          updatedOrderdetails.product[0].quantity;
                        await UserServices.updateRecord(
                          { _id: ObjectID(user_id) },
                          { $inc: { wallet: parseFloat(returnamount) } }
                        ).then(async (updatedWallet) => {
                          if (updatedWallet) {
                            var addPattern = {};
                            var transition_id = Common.generateRandomNumber(8);
          ...
          $project: {
            ...
            products: {
              $filter: {
                ...
                cond: {
                  $eq: [
                    "$$num.business_category_id",
                    ObjectID(business_category_id),
                  ],
                },
              },
            },
      ...
      OrderServices.oneRecord(aggregateCondition)
        .then(async (orderDetail) => {
          ...
          if (orderDetail && orderDetail.length > 0) {
            ...
            var getresult = await Promise.all(
              orderData.products.map(async (val) => {
                ...
                await OrderServices.oneRecord([
                  {
                    $match: {
                      _id: ObjectID(order_id),
                      user_id: ObjectID(user_id),
                      products: { $elemMatch: { _id: ObjectID(val._id) } },
                    },
                  },
                  {
                    $project: {
                  ...
                  .then(async (orderUpdate) => {
                    if (orderUpdate[0].product[0].status == 0) {
                      var returnamount =
                        orderUpdate[0].product[0].is_discount == 1
                          ? orderUpdate[0].product[0].offer_price
                          : orderUpdate[0].product[0].price;
                      newwalletamount =
                        newwalletamount +
                        parseFloat(
                          returnamount * orderUpdate[0].product[0].quantity
                        );
                      await OrderServices.updateRecord(
                        {
                          _id: ObjectID(order_id),
            ...
            if (orderData.payment_mode != "Cash") {
              await UserServices.updateRecord(
                { _id: ObjectID(user_id) },
                { $inc: { wallet: parseFloat(newwalletamount) } }
              ).then(async (updatedWallet) => {
                if (updatedWallet) {
                  var addPattern = {};
                  var transition_id = Common.generateRandomNumber(8);
      ...
      OrderServices.oneRecord(aggregateCondition)
        .then(async (orderDetail) => {
          ...
          if (orderDetail && orderDetail.length > 0) {
            ...
            let OrderReturnTime = new Date(
              createTime.getTime() + bussinessCatData.return_time * 60000
            );
                ...
                if (orderUpdate[0].product[0].status != 1) {
                  await OrderServices.updateRecord(
                    ...
                    .then(async (orderUpdate) => {
                      ...
                      var notificationTitle = "Athwas";
                      var notificationText =
                        "The return request of order #" +
                        orderUpdate.order_id +
                        " has been requested successfully";
                      var adminnotificationText =
                        "New return request of order #" +
                        orderUpdate.order_id +
  ...
  getRedeemPoints: async (req, res) => {
    try {
      ...
      RedeemPointServices.getAggregatePaginatedData(
        aggregatefilter,
        sortPattern,
        page,
        limit
      )
        .then(async (pointsData) => {
          let resMsg = "Success";
          let userData = await UserServices.getUserDetailById({
            _id: login_user_id,
          });
          if (pointsData && pointsData.docs && pointsData.docs.length > 0) {
            var normalsetting = await contentsettings.getRecordsList();

            pointsData.docs.map((item, index) => {
              var date1 = new Date().getTime();
              var date2 =
                new Date(item.createdAt).getTime() +
                normalsetting[0].expiration_time * 24 * 60 * 60 * 1000;

              console.log("date1=", date1, "date2", date2);
              item.isExpired = date1 < date2 ? 0 : 1;
  ...
  genrateOrderforOnlinePayment: async (req, res) => {
    ...
    if (!payment_mode || payment_mode == "") {
      errors.push({
        errField: "payment_mode",
        errText: "payment_mode field is empty.",
      });
    }
    if (!delivery_address || delivery_address == "") {
      errors.push({
        errField: "delivery_address",
        errText: "delivery_address field is empty.",
      });
    }
    // return errors
    if (errors.length > 0) {
      ...
      await OrderServices.createRecord(updaterecord)
        .then((orderResult) => {
    ...
    try {
      ...
      if (errors.length > 0) {
        ...
        if (orderDetail && orderDetail.length > 0) {
          var filePath =
            config.rootPath +
            "/src/views/email_templates/order_detail1_new_design.ejs";
          let customData = orderDetail[0];
          customData["moment"] = moment;
          customData["logo"] =
            config.apiGatewayUrl +
            "/service/assets?service=user_service&filepath=default/black-logo.png";
          customData["newstatus1"] = [
            "",
            "Assigned",
            "Completed",
            "Rejected",
          ...
          customData["amountInWord"] = await Common.inWords(
            Math.round(customData.net_amount)
          );
          let normal_setting = await ContentServices.getRequest(
            "customer/normal_settings"
          );
          ...
          ejs.renderFile(filePath, customData, (err, data) => {
            if (err) {
              ...
              pdf
                .create(data, options)
                .toFile(`public/generated-pdf/${path}`, (err, resp) => {
                  if (err) {
                    return console.log("error");
                  }
                  let sendData = {
                    path: config.siteUrl + "/generated-pdf/" + path,
                  };
