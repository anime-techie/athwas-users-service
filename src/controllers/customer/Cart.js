...
const CartController = {
  ...
  //     var cartProduct = await CartServices.oneRecord([{ $match: CreateCart }]);

  //     var productavailable = await ProductServices.findOne(findProduct);

  //     if (productavailable == null) {
  //       let Message = "Product is not available";
  //       Response.send(req, res, 400, Message);
  //     } else {
  //       CartServices.createOrUpdateRecord(CreateCart, quantity)
  ...
  //           }
  //           var cart_count = {
  //             cart_count: await CartServices.getDataCount({
  //               user_id: login_user_id,
  //             }),
  //           };
  //           console.log(cartResult, cart_count);
  ...
  addToCart: async (req, res) => {
    try {
      ...
      if (productavailable == null) {
        let Message = "Product is not available";
        Response.send(req, res, 400, Message);
      } else if (cartProduct && cartProduct.length > 0) {
        var cart_count = {
          cart_count: await CartServices.getDataCount({
            user_id: login_user_id,
          }),
        };
        let Message = Messages.CARTITEM_ALREADY_ADDED;
        Response.send(req, res, 200, Message, cart_count);
      } else {
        CartServices.createOrUpdateRecord(CreateCart, quantity)
          .then(async (cartResult) => {
            var succMessage = "";
  ...
  deleteCartItem: async (req, res) => {
    try {
      ...
      CartServices.deleteRecord(CreateCart)
        .then(async (cartResult) => {
          //console.log(cartResult);
          var cart_count = {
            cart_count: await CartServices.getDataCount({
  ...
  */
  ...
  getCartItems: async (req, res) => {
    ...
    CartServices.getAggregatePaginatedData(
      aggregateFilter,
      sortPattern,
      page,
      limit
