    ...
    */
	get_subcategories: (req, res) => {
		try {
			...
			if (keyword && keyword != '') {
				findPattern['$or'] = [{ name: { $regex: keyword, $options: 'i' } }];
			}
			...
			CategoryServices.getPaginatedData(findPattern, sortPattern, page, limit)
				.then((categorydata) => {
					let resMsg = 'Success';
	...
	create_subcategory: async (req, res) => {
		try {
			...
			if (errors.length > 0) {
				...
				var findPattern = {
					business_category_id: mongoose.Types.ObjectId(business_category_id),
					parent_id: parent_id,
					name: { $regex: name, $options: 'i' },
				};
				//s
				CategoryServices.oneRecord(findPattern)
					.then(async (categorydata) => {
						if (categorydata) {
							Response.send(
								...
								res,
								400,
								'Sub Category already exist with this business category and parent category.',
								{ errors: errors },
							);
						} else {
							var createPattern = {
							...
							CategoryServices.createRecord(createPattern)
								.then(async (createRes) => {
									var findPattern = { _id: createRes._id };
									var updatePattern = {};

									if (files && files.image_path) {
										var fileName = await CategoryServices.updateUserImage(
											createRes,
											files.image_path,
										);
									...
									CategoryServices.updateRecord(findPattern, updatePattern)
										.then((updatedRes) => {
											// success
		...
		try {
			...
			if (errors.length > 0) {
				...
				CategoryServices.oneRecord(findPattern)
					.then(async (categorydata) => {
						if (categorydata) {
							var checkExistPattern = {
								business_category_id:
									mongoose.Types.ObjectId(business_category_id),
								parent_id: parent_id,
								name: { $regex: name, $options: 'i' },
								_id: { $ne: subcategory_id },
							};
							let checkName = await CategoryServices.oneRecord(
								checkExistPattern,
							);
							if (checkName) {
    ...
    */
	get_category_by_business: (req, res) => {
		try {
			...
			CategoryServices.allRecord(findPattern)
				.then((categorydata) => {
					if (categorydata) {
						let resMsg = 'Success';
						Response.send(req, res, 200, resMsg, categorydata);
