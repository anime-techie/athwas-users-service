...
const BrandController = {
  ...
  create_brand: async (req, res) => {
    try {
      ...
      if (name) {
        var findPattern = { name: name.trim(), is_deleted: 0 };
        var itemData = await BrandServices.oneRecord(findPattern);

        if (itemData) {
          errors.push({
            errField: "name",
            errText: Messages.BRAND_NAME_ALREADY_USED,
          });
        }
      }
      ...
      if (errors.length > 0) {
        ...
        BrandServices.createRecord(createPattern)
          .then(async (createRes) => {
            var findPattern = { _id: createRes._id };
            var updatePattern = {};

            if (files && files.image_path) {
              var fileName = await BrandServices.updateUserImage(
                createRes,
                files.image_path
              );
            ...
            BrandServices.updateRecord(findPattern, updatePattern)
              .then((updatedRes) => {
                // success
  ...
  get_brand: async (req, res) => {
    try {
      ...
      BrandServices.oneRecord(findPattern)
        .then((branddata) => {
          if (branddata) {
            let resMsg = "Success";
            Response.send(req, res, 200, resMsg, branddata);
      ...
      BrandServices.oneRecord(findPattern)
        .then((branddata) => {
          if (branddata != null) {
            ...
            BrandServices.updateRecord({ _id: branddata._id }, updatePattern)
              .then((userObjRes) => {
