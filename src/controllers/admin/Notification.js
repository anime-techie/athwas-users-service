...
const NotificationController = {
      ...
      OrderServices.getAggregatePaginatedData(
        ...
        .then(async (userdata) => {
          if (userdata) {
            ...
            UserServices.allRecord(aggregateCondition)
              .then((userdata) => {
                let resMsg = "Success";
    ...
    try {
      ...
      if (req.body.sent_to == "all_users") {
        UserServices.allRecord(aggregateCondition)
          .then(async (userdata) => {
            (await userdata) &&
              userdata.reduce(async (promise, user, key) => {
                await promise;
  ...
  getNotifications: async (req, res) => {
    try {
      ...
      var NotificationCount = await NotificationServices.updateRecordMany(
        { user_type: 1 },
        { read_status: 1 }
      );

      await NotificationServices.getAggregatePaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
