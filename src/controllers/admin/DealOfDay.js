  ...
  create_dealofday: async (req, res) => {
    try {
      ...
      if (
        files &&
        files.image &&
        Media.getMediaTypeByMimeType(files.image.mimetype) != "image"
      ) {
        errors.push({
          errField: "image",
      ...
      if (errors.length > 0) {
        ...
        DealOfDayServices.createRecord(createPattern)
          .then(async (createRes) => {
            var findPattern = { _id: createRes._id };
            var updatePattern = {};

            if (files && files.image) {
              var fileName = await DealOfDayServices.updateUserImage(
                createRes,
                files.image
              );
            ...
            DealOfDayServices.updateRecord(findPattern, updatePattern)
              .then((updatedRes) => {
                // success
      ...
      DealOfDayServices.oneRecord(aggregateCondition)
        .then((bannerdata) => {
          if (bannerdata != null) {
            ...
            DealOfDayServices.updateRecord(
              { _id: bannerdata._id },
              updatePattern
            )
              .then((userObjRes) => {
