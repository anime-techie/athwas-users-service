    ...
    try {
      ...
      if (files && files.product_images) {
        if (Array.isArray(files.product_images)) {
          ...
          if (
            Media.getMediaTypeByMimeType(files.product_images.mimetype) !=
            "image"
          ) {
            errors.push({
              errField: files.product_images.name,
              errText: Messages.INVALID_IMAGE_FORMAT,
            });
          }
        }
      ...
      if (errors.length > 0) {
        ...
        ProductServices.createRecord(createProduct)
          .then(async (productResult) => {
            var product_id = await ProductServices.genrateproductcode(
              productResult._id
            );
            ...
            ProductInventoryServices.createRecord(createProductInventry)
              .then(async (resultInventry) => {
                await customize.reduce(async (promise, customize, key) => {
                  ...
                  await ProductCustomizationServices.createRecord(
                    createProductInventry
                  )
                    .then(async (customizersult) => {})
                    .catch((err) => {
                ...
                await warehouse_inventory.reduce(
                  async (promise, warehouse_inventory, key) => {
                    await promise;
            ...
            if (files && files.product_images) {
              if (Array.isArray(files.product_images)) {
                await files.product_images.reduce(
                  async (promise, arrayItem) => {
                    await promise;
                    if (
                      Media.getMediaTypeByMimeType(arrayItem.mimetype) !=
                      "image"
                    ) {
                      errors.push({
                        errField: arrayItem.name,
                      ...
                      var fileName = await ProductServices.updateUserImage(
                        updateResult,
                        arrayItem
                      );
  ...
  */
  ...
  getProductLists: async (req, res) => {
    try {
      ...
      ProductServices.getAggregatePaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
  ...
  updateStatus: (req, res) => {
    try {
      ...
      var aggregateCondition = [{ $match: findPattern }];
      ProductServices.oneRecord(aggregateCondition)
        .then((productdata) => {
  ...
  getCustomizationTypes: (req, res) => {
    try {
      ...
      CustomizationTypeServices.getPaginatedData(
        aggregateCondition,
        sortPattern,
        page,
        limit
      )
        .then((customizedatadata) => {
          let resMsg = "Success";
  ...
  getCustomizationTypeValues: (req, res) => {
    try {
      ...
      CustomizationTypeServices.allRecord([{ $match: findPattern }])
        .then((typeResult) => {
  ...
  getProductDetail: async (req, res) => {
    try {
      ...
      ProductServices.oneRecord(aggregatefilter)
        .then((productData) => {
          if (productData) {
            let resMsg = "Success";
        ...
        .then((productdata) => {
          ...
          if (productImages != null) {
            ...
            if (index.toString() != "") {
              ...
              ProductServices.updateRecord(findPattern, updateProduct)
                .then((updatedProduct) => {
        ...
        ProductServices.oneRecord(aggregateCondition)
          .then(async (productdata) => {
            if (productdata) {
              ...
              ProductServices.updateRecord(findPattern, updatePattern)
                .then(async (updatedRes) => {
                  ...
                  if (files && files.product_images) {
                    if (Array.isArray(files.product_images)) {
                      await files.product_images.reduce(
                        async (promise, arrayItem) => {
                          ...
                          if (
                            ...
                            var fileName =
                              await ProductServices.updateUserImage(
                                updateResult,
                                arrayItem
                              );
                            image_name.push(fileName);
                      ...
                      if (
                        Media.getMediaTypeByMimeType(
                          files.product_images.mimetype
                        ) != "image"
                      ) {
                        errors.push({
                          errField: files.product_images.name,
    ...
    try {
      ...
      if (inventory_id != "") {
        ...
        if (errors.length > 0) {
          ...
          ProductInventoryServices.oneRecord(aggregateCondition)
                  ...
                  .then(async (updatedRes) => {
                    ...
                    await customize.reduce(async (promise, customize, key) => {
                      ...
                      if (customize.id != "") {
                        ...
                        await ProductCustomizationServices.updateRecord(
                          { _id: customize.id },
                          updateProductInventry
                        )
                          .then(async (updatedRes) => {
                            return;
                          })
                    ...
                    await warehouse_inventory.reduce(
                      async (promise, newinventory, key) => {
                        await promise;
                        if (newinventory.id != "") {
                          var newfindPattern = {
                            _id: ObjectID(inventory_id),
                            is_deleted: 0,
                            "warehouseinventry._id": ObjectID(newinventory.id),
                          };

                          var updateProductInventry = {
                            "warehouseinventry.$.warehouse_id":
                              newinventory.warehouse_id,
        ...
        var addProductCustomization =
          await ProductInventoryServices.createRecord(createProductInventry)
            .then(async (resultInventry) => {
              ...
              await warehouse_inventory.reduce(
                async (promise, newinventory, key) => {
                  ...
                  await ProductInventoryServices.updateRecord(
                    { _id: ObjectID(resultInventry._id) },
                    { $push: { warehouseinventry: createProductInventry } }
                  )
                    .then(async (customizersult) => {
                      return true;
  ...
  deleteCustomizeData: (req, res) => {
    try {
      ...
      ProductCustomizationServices.oneRecord(aggregateCondition)
        .then((customizedata) => {
          if (customizedata != null) {
            ...
            ProductCustomizationServices.updateRecord(
              { _id: findPattern },
              updatePattern
            )
              .then((userObjRes) => {
      ...
      ProductInventoryServices.oneRecord(aggregateCondition)
        .then((inventorydata) => {
          if (inventorydata != null) {
            ...
            ProductInventoryServices.updateRecord(
              { _id: inventorydata._id },
              updatePattern
            )
              .then((userObjRes) => {
                ProductCustomizationServices.updateRecord(
                  { inventory_id: userObjRes._id },
                  updatePattern
                )
                  .then((userObjRes) => {
                    let resMsg = Messages.CUSTOMIZE_DATA_DELETE_SUCCESS;
  ...
  deleteWarehouseInventoryData: (req, res) => {
    try {
      ...
      ProductInventoryServices.updateRecord(
        { _id: ObjectID(inventory_id) },
        { $pull: { warehouseinventry: { _id: warehouseinventory_id } } }
      )
        .then((userObjRes) => {
