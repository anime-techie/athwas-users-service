...
const BusinessCategoryController = {
  ...
  create_business_category: async (req, res) => {
    try {
      ...
      if (
        ...
        Media.getMediaTypeByMimeType(files.category_image.mimetype) != "image"
      ) {
        errors.push({
          errField: "category_image",
          errText: Messages.INVALID_IMAGE_FORMAT,
        });
      }
      let findBuisnessCategory = await BusinessCategoryServices.allRecord({
      ...
      if (errors.length > 0) {
        ...
        if (findBuisnessCategory.length > 0) {
          let resMsg = Messages.BUSINESS_CATEGORY_ALREADY_EXIST;
          Response.send(req, res, 500, resMsg);
        }
        BusinessCategoryServices.createRecord(createPattern)
          .then(async (createRes) => {
            var findPattern = { _id: createRes._id };
            var updatePattern = {};

            if (files && files.category_image) {
              var fileName = await BusinessCategoryServices.updateUserImage(
                createRes,
                files.category_image
              );
            ...
            BusinessCategoryServices.updateRecord(findPattern, updatePattern)
              .then((updatedRes) => {
                // success
  ...
  get_business_category: async (req, res) => {
    try {
      ...
      BusinessCategoryServices.oneRecord(findPattern)
        .then((categorydata) => {
          if (categorydata) {
            let resMsg = "Success";
            Response.send(req, res, 200, resMsg, categorydata);
      ...
      BusinessCategoryServices.oneRecord(findPattern)
        .then((categorydata) => {
          if (categorydata != null) {
            ...
            BusinessCategoryServices.updateRecord(
