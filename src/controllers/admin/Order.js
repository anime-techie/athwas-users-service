...
const OrderController = {
        ...
        { $unwind: "$delivery_address" },
        {
          $project: {
            ...
            delivery_address: {
              ...
              address_location: "$delivery_address.geoLocation",
  ...
  */
  ...
  assignOrder: async (req, res) => {
    ...
    // var checkuserDeleted = orderIdsArr.map(async (val) => {
    //   var ordetail = await Order.findOne({ _id: val });
    //   if (ordetail) {
    //     var userDetail = await UserServices.getUserDetailById({
    //       _id: ordetail.user_id,
    //     });
    ...
    if (checkDriverOutForDelivery) {
      ...
      if (orderIdsArr.length > driverMaxOrderAllow) {
        let resMsg =
          "You cannot assign more than 10 orders to a Driver at a time";

        Response.send(req, res, 500, resMsg);
      } else if (existOrder + orderIdsArr.length > driverMaxOrderAllow) {
        let resMsg =
          "Driver has already been assigned with 10 orders. You cannot assign more than 10 orders to a Driver at a time";
        ...
        try {
          // console.log("bbbbbbbbbbbbbbbbbbbbbbbb", orderIdsArr);
          Order.updateMany(
            { _id: { $in: orderIdsArr } },
            {
              $set: {
            ...
            .then(function async(data) {
              if (userInfo.device_token != "") {
                ...
                if (userInfo.device_type == 1) {
                  message = {
                    ...
                    data: {
                      ...
                      title: "Athwas",
                      body:
                        orderIdsArr.length + " " + "order has been assigned",
                    },
                  };
                } else {
                  message = {
                ...
                //     }
                // };
                let order_length = orderIdsArr.length + " orders ";
                if (orderIdsArr.length == 1) {
                  order_length = "1 order ";
                }
                console.log("===========>11", order_length);
                // PushNotification.sendForAndriodIos(message);
                NotificationServices.sendNotification(
                  "Athwas",
                  order_length + "has been assigned",
                  driver_id,
                  3,
                  "",
                  "",
                    ...
                    await OrderServices.updateRecord(
                      {
                        ...
                        user_id: ObjectID(user_id),
                        products: {
                          $elemMatch: { _id: ObjectID(sub_order_id) },
                        },
                      },
                      ...
                      .then(async (orderUpdate) => {
                        ...
                        returnamount = parseFloat(
                          returnamount * updatedOrderdetails.product[0].quantity
                        );
                        ...
                        await OrderServices.updateRecord(
                          {
                            ...
                            user_id: ObjectID(user_id),
                            products: {
                              $elemMatch: { _id: ObjectID(val._id) },
                            },
                          },
                          {
