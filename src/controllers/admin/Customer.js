...
const UPLOAD_FOLDER_URL =
  ...
  "/service/assets?service=user_service&filepath=uploads/users";
  ...
  get_users: async (req, res) => {
    try {
      ...
      if (daysLimit != "" && daysLimit != 0) {
        ...
        // findOrderPattern["$and"].push({
        //   createdAt: { $lt: new Date(endDate) },
        // });myDate.toISOString()
        // findOrderPattern = {
        //   createdAt: {
        //     $gte: new Date("2021-10-27T05:24:50.971").toISOString(),
        //     // $lt: new Date(),
        //   },
        // };
        findOrderPattern["$and"] = [
          {
            createdAt: { $gte: "2021-10-27T05:24:50.971" },
          },
          // ""  { createdAt: { $lte: "2022-02-12T05:24:50.971" } },
        ];
        console.log(JSON.stringify(findOrderPattern));
      }
      ...
      if (daysLimit != "" || amountLimit != 0) {
        ...
        console.log("aggregateOrderCondition", aggregateOrderCondition);
    ...
    try {
      ...
      if (address != "") {
        ...
        if (!cordinates || cordinates.length == 0) {
          errors.push({
            errField: "address",
            errText: Messages.INVALID_ADDRESS,
          });
        }
      }
      ...
      if (errors.length > 0) {
        ...
        UserServices.oneRecord(aggregateConditions)
          .then((userdata) => {
            if (
              userdata &&
              ...
              var createPattern = {
                ...
                phone: phone,
                password: password
                  ? Encryption.getBcryptEncryption(password)
                  : "",
                user_image: "",
              };
              ...
              UserServices.createRecord(createPattern)
                .then(async (createRes) => {
                  ...
                  if (files && files.user_image) {
                    var fileName = await UserServices.updateUserImage(
                      createRes,
                      files.user_image
                    );
        ...
        UserServices.oneRecord(aggregateConditions)
          .then(async (userdata) => {
            if (userdata) {
              ...
              UserServices.oneRecord(newaggregateConditions).then(
                async (newuserdata) => {
                  if (
                    newuserdata &&
                    newuserdata.phone == phone &&
                    newuserdata._id != user_id
                  ) {
                    var resMsg = Messages.PHONE_ALREADY_REGISTERED;
                    Response.send(req, res, 400, resMsg);
                  } else if (
      ...
      UserServices.oneRecord(aggregateConditions)
        .then((userdata) => {
          if (userdata != null) {
            ...
            UserServices.updateRecord({ _id: userdata._id }, updatePattern)
              .then((userObjRes) => {
                if (status == 0) {
...
*/
  delete_user: async (req, res) => {
    try {
      ...
      let findOrderPattern = {
        user_id: ObjectID(user_id),
        order_status: 1,
        driver_id: { $nin: [null, ""] },
      };
      let aggregateConditionsOrders = [
        {
          $match: findOrderPattern,
        },
      ];

      let userOrders = await OrderServices.allRecord(aggregateConditionsOrders);

      if (userOrders && userOrders.length > 0) {
        let resMsg = Messages.USER_CANNOT_DELETE;
        Response.send(req, res, 400, resMsg);
      } else {
        UserServices.oneRecord(aggregateConditions)
          .then((userdata) => {
