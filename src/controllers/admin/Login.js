    ...
    */
	admin_login: (req, res) => {
		try {
			...
			AdminServices.oneRecord(findPattern)
				.then((admindata) => {
					if (admindata) {
						...
						if (checkRes) {
							...
							if (admindata.is_active == 0) {
								...
								AdminServices.updateRecord(findPattern, updatePattern).then(
									(updateRes) => {
										var key = updateRes.user_role + '_' + updateRes._id;
										Redis.hmset(
											['_auth_tokens', key, updateRes.auth_token],
											function (err, res) {
	...
	update_profile: (req, res) => {
		try {
			...
			if (
				files &&
				files.user_image &&
				Media.getMediaTypeByMimeType(files.user_image.mimetype) != 'image'
			) {
				errors.push({
					errField: 'user_image',
			...
			if (errors.length > 0) {
				...
				var findPattern = { _id: login_user_id, is_deleted: 0 };
				AdminServices.getAdminData(findPattern)
					.then(async (admindata) => {
						if (admindata) {
							...
							if (files && files.user_image) {
								var fileName = await AdminServices.updateUserImage(
									admindata,
									files.user_image,
								);
								updatePattern.user_image = fileName;
							}
							AdminServices.updateRecord(findPattern, updatePattern)
								.then((updatedRes) => {
									// success
	...
	change_password: (req, res) => {
		try {
			...
			if (!old_password || old_password == '') {
				errors.push({
					errField: 'old_password',
					errText: 'Old Password field is empty.',
				});
			}
									...
									if (otpCheckRes && otpCheckRes.otp_number == otp_number) {
										OtpServices.deleteRecord(findOtpPattern)
											.then((otpDeleteRes) => {
												var updatePattern = {
													password:
														Encryption.getBcryptEncryption(new_password),
												};
    ...
    */
	getDashboardData: async (req, res) => {
		try {
			...
			var findCustomerPattern = {
				...datePattern,
				...{ is_deleted: 0, user_role: 3 },
			};
			var findDriverPattern = {
				...datePattern,
				...{ is_deleted: 0, user_role: 4 },
			};
			var findBrandPattern = { ...datePattern, ...{ is_deleted: 0 } };
			var findWarehousePattern = { ...datePattern, ...{ is_deleted: 0 } };
			var findBusinessCategoryPattern = {
				...datePattern,
				...{ is_deleted: 0 },
			};
			...
			var warehouseCount = await WarehouseServices.getDataCount(
				findWarehousePattern,
			);
			var businessCategoryCount = await BusinessCategoryServices.getDataCount(
				findBusinessCategoryPattern,
			);
			var productCategoryCount = await CategoryServices.getDataCount(
				findProductCategoryPattern,
			);
			...
			var NotificationCount = await NotificationServices.getDataCount({
				user_type: 1,
				read_status: 0,
			});
			// success
