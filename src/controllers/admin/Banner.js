...
const BannerController = {
  ...
  create_banner: async (req, res) => {
    try {
      ...
      if (
        files &&
        files.banner_image &&
        Media.getMediaTypeByMimeType(files.banner_image.mimetype) != "image"
      ) {
        errors.push({
          errField: "banner_image",
      ...
      if (errors.length > 0) {
        ...
        BannerServices.createRecord(createPattern)
          .then(async (createRes) => {
            var findPattern = { _id: createRes._id };
            var updatePattern = {};

            if (files && files.banner_image) {
              var fileName = await BannerServices.updateUserImage(
                createRes,
                files.banner_image
              );
            ...
            BannerServices.updateRecord(findPattern, updatePattern)
              .then((updatedRes) => {
                // success
  ...
  getBanner: async (req, res) => {
    try {
      ...
      BannerServices.oneRecord(aggregateCondition)
        .then((userdata) => {
          let resMsg = "Success";
          Response.send(req, res, 200, resMsg, userdata);
        })
      ...
      BannerServices.oneRecord(aggregateCondition)
        .then((bannerdata) => {
          if (bannerdata != null) {
            ...
            BannerServices.updateRecord({ _id: bannerdata._id }, updatePattern)
              .then((userObjRes) => {
  ...
  getBannerProducts: async (req, res) => {
    try {
      ...
      var aggregateCondition = [
        { $match: findPattern },
        {
          $lookup: {
            from: "product_inventories",
            let: { id: "$_id" },
            pipeline: [{ $match: { $expr: { $eq: ["$product_id", "$$id"] } } }],
      ...
      ProductServices.allRecord(aggregateCondition)
        .then((userdata) => {
          let resMsg = "Success";
          Response.send(req, res, 200, resMsg, userdata);
          // });
