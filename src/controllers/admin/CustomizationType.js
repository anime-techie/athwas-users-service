...
const CustomizationTypeController = {
  ...
  createCustomizationType: async (req, res) => {
    try {
      ...
      if (errors.length > 0) {
        ...
        CustomizationTypeServices.oneRecord(myAggregate)
          .then((matchData) => {
            if (matchData) {
              ...
              CustomizationTypeServices.createRecord(createPattern)
  ...
  getCustomizationType: async (req, res) => {
    try {
      ...
      CustomizationTypeServices.oneRecord(aggregateConditions)
        .then((typedata) => {
          if (typedata) {
            let resMsg = "Success";
  ...
  update_customize_type: async (req, res) => {
    try {
      ...
      if (errors.length > 0) {
        ...
        CustomizationTypeServices.oneRecord(aggregateCondition)
          .then(async (typedata) => {
            if (!typedata) {
              var updatePattern = {
                name: name,
              ...
              CustomizationTypeServices.updateRecord(
                { _id: type_id },
                updatePattern
              )
                .then((updatedRes) => {
      ...
      CustomizationTypeServices.oneRecord(aggregateCondition)
        .then((categorydata) => {
          if (categorydata != null) {
            CustomizationTypeServices.deleteRecord({ _id: categorydata._id })
              .then((userObjRes) => {
