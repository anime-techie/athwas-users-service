...
const { OfferServices, ProductInventoryServices } = require("../../services");
...
const OfferController = {
  ...
  createOffer: async (req, res) => {
    try {
      ...
      var product_id = req.body.product_id
        ? JSON.parse(req.body.product_id)
        : [];
      ...
      if (offer_type == 1) {
        if (coupon_code == "") {
          errors.push({
            errField: "coupon_code",
            errText: "Please enter coupon code",
          });
        }
      }
      ...
      if (offer_type == 2 || offer_type == 1 || offer_type == 4) {
        if (offer_amount_type == "") {
          errors.push({
            errField: "offer_amount_type",
            errText: "Please select Offer amount type",
          });
        }
        if (offer_price == "") {
          errors.push({
            errField: "offer_price",
            errText: "Please enter price",
          });
        }
        if (offer_amount == "") {
          errors.push({
            errField: "offer_amount",
            errText: "Please enter offer amount",
          });
        }
      }
      if (offer_type == 4) {
        if (card_type == "") {
          errors.push({
            errField: "card_type",
            errText: "Please select card type",
          });
        }
        if (bank_type == "") {
          errors.push({
            errField: "bank_type",
            errText: "Please select bank type",
          });
        }
      }
      if (offer_type == 3) {
        if (offer_price == "") {
          errors.push({
            errField: "offer_price",
            errText: "Please select offer price",
          });
        }
        if (offer_quantity == "") {
          errors.push({
            errField: "offer_quantity",
            errText: "Please enter offer quantity",
          });
        }
      }
      if (
        files &&
        files.image_path &&
        Media.getMediaTypeByMimeType(files.image_path.mimetype) != "image"
      ) {
        errors.push({
          errField: "image_path",
      ...
      if (
        ...
        let productData = await ProductInventoryServices.oneRecord([
          { $match: { _id: ObjectID(product_id[0]) } },
          {
            $project: {
              price: 1,
              product_id: 1,
              is_discount: 1,
              discounted_product_price: 1,
            },
          },
        ]);

        if (
          productData.is_discount == 1 &&
          productData.discounted_product_price
        ) {
          let productAmount =
            productData.discounted_product_price * offer_quantity;
          if (productAmount < offer_price) {
            let resMsg = Messages.OFFER_PRICE_MUST_OFFER_AMOUT;
            Response.send(req, res, 400, resMsg);
          }
        } else {
          let productAmount = productData.price * offer_quantity;
          if (productAmount < offer_price) {
            let resMsg = Messages.OFFER_PRICE_MUST_OFFER_AMOUT;
            Response.send(req, res, 400, resMsg);
          }
      ...
      if (errors.length > 0) {
        ...
        if (coupon_code && coupon_code != "") {
          ...
          findPattern.coupon_code = { $regex: coupon_code, $options: "i" };
        } else {
        ...
        OfferServices.oneRecord(aggregateConditions).then(async (matchdata) => {
          if (matchdata) {
            ...
            OfferServices.createRecord(createPattern)
              .then(async (createRes) => {
                var findPattern = { _id: createRes._id };
                var updatePattern = {};

                if (files && files.image_path) {
                  var fileName = await OfferServices.updateUserImage(
                    createRes,
                    files.image_path
                  );
                ...
                OfferServices.updateRecord(findPattern, updatePattern)
                  .then((updatedRes) => {
                    // success
  ...
  getOffer: async (req, res) => {
    try {
      ...
      OfferServices.oneRecord(aggregateCondition)
        .then((userdata) => {
          let resMsg = "Success";
          Response.send(req, res, 200, resMsg, userdata);
        })
  ...
  updateOffer: async (req, res) => {
    try {
      ...
      var category_id = req.body.category_id || null;
      var sub_category_id = req.body.sub_category_id || null;
      var product_id = JSON.parse(req.body.product_id) || [];
      var card_type = req.body.card_type || "";
      var bank_type = req.body.bank_type || "";
      var start_date = req.body.startDate || "";
      var end_date = req.body.endDate || "";
      ...
      if (errors.length > 0) {
        ...
        if (coupon_code && coupon_code != "") {
          findExistPattern.coupon_code = { $regex: coupon_code, $options: "i" };
        } else {
          if (business_category_id && business_category_id != "") {
            findExistPattern.business_category_id =
      ...
      OfferServices.oneRecord(aggregateCondition)
        .then((offerdata) => {
          if (offerdata != null) {
            ...
            OfferServices.updateRecord({ _id: offerdata._id }, updatePattern)
              .then((userObjRes) => {
      ...
      OfferServices.oneRecord(aggregateCondition)
        .then((bannerdata) => {
          if (bannerdata != null) {
            ...
            OfferServices.updateRecord({ _id: bannerdata._id }, updatePattern)
              .then((userObjRes) => {
